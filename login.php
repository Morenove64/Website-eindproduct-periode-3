		...
		{ 
			...
			if($query->rowCount()==1) 
			{
				...
				{			
					...
					$_SESSION['idklant']=$rij['idklant'];
					$_SESSION['voornaam']=$rij['voornaam'];
					$_SESSION['achternaam']=$rij['achternaam'];
					header('Refresh: 0; url=bootstrap2.php');										
				}
				else 
				{
					header('Refresh: 3; url=loginform.php'); 
					echo "Combinatie van klantid en wachtwoord is onjuist";
				}
			}
			else
			{
				header('Refresh: 3; url=loginform.php');
				echo "Klant met dit nummer bestaat niet";
			}
		} 
