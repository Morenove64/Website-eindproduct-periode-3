...
<head>
	...
	<body class="vraag4">
		...
		try
		{
			...
			$query = $db->prepare("SELECT product.naam, verkoopdatum, verkoop.aantal FROM verkoop INNER JOIN product ON verkoop.idproduct = product.idproduct GROUP BY verkoopdatum ORDER BY aantal DESC ");
			$query->execute();
			$result = $query->fetchALL (PDO::FETCH_ASSOC);
			echo '<table class="table table-dark table-striped table-hover table-bordered">';
				echo '<thead>';
					echo '<td>Naam van Product:</td>';
					echo '<td>Verkoop Datum:</td>';
